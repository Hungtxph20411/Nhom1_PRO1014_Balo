...
public class SanPhamChiTiet_Service {
    ...
    public Integer Them_SanPhamChiTiet(SanPhamChiTiet maSPCT) {
        sql = " insert into SanPhamChiTiet(MaSPCT ,IDSanPham,IDMauSac,IDChatLieu,IDSoNgan,IDSize,IDQuaiDeo,IDHang,Gia,SoLuong,TinhTrang) values(?,?,?,?,?,?,?,?,?,?,?)";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
            ...
            ps.setString(11, maSPCT.getTinhTrang());
    ...
    public Integer Sua_SanPhamChiTiet(SanPhamChiTiet maSPCT, String ma) {
        sql = " update SanPhamChiTiet set IDSanPham = ?,IDMauSac =?,IDChatLieu = ?,IDSoNgan = ?,IDSize = ?,IDQuaiDeo = ?,IDHang = ?,Gia = ?,SoLuong= ? where MaSPCT = ?";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
...
//            ps.setString(10, maSPCT.getTinhTrang());
    ...
    public Integer Xoa_SanPhamChiTiet(String ma) {
        sql = " Delete from SanPhamChiTiet where MaSPCT = ?";
        try {
            con = DBConnect.getConnection();
            ps = con.prepareStatement(sql);
            ps.setString(1, ma);
